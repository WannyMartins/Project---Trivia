import tokenAPI from '../reducers/token';
...
export const tokenRequestSucess = (tok) => ({ // Função para dizer que foi um sucesso a requisição da API
  type: REQUEST_API_TOKEN_SUCESS,
  token: tok,
});

export const tokenRequestAPI = () => (dispatch) => { // Despacha para o store o resultado da requisição
  dispatch(tokenRequest()); // Invoca o primeiro Type
  try {
    // console.log('tokenRequest');
    const request = tokenAPI(); // Pesquisa na URL a moeda
